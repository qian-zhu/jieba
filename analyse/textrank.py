...
def textrank(sentence, topK=10, withWeight=False,stopWords=[],span=5):
    """
    ...
    # At least two vertexes make up a graph
    effective_words = []
    for i in xrange(words_length):
        if words[i].flag in pos_filt and words[i].word not in stopWords:
            effective_words.append(words[i].word)
            for j in xrange(i + 1, i + span):
                if j >= words_length:
                    break
                if words[j].flag not in pos_filt:
                    continue
                if words[j].word in stopWords:
                    continue
    ...
    if cm: 
        ...
        if withWeight:
            tags = [(x,1) for x in effective_words]
        else:
            tags = effective_words
        if topK:
            return tags[:topK]
        else:
            return tags
